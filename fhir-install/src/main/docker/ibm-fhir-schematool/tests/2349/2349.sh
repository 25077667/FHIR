...
docker-compose run tool --tool.behavior=onboard --db.host=192.168.86.29 --db.port=50000 --user=db2inst1 --password=hey_yoh_what --db.database=fhirdb --sslConnection=false --db.type=db2 --schema.name.fhir=CLINIC --grant.to=testuser     --tenant.name=CLINICA
